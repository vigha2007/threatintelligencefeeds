  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const { data, error } = await auth.supabase.from(def.key).select("*").eq("id", id.data).maybeSingle();
      ...
      DELETE: async ({ request, params }) => {
        ...
        if (auth instanceof Response) return auth;
        const def = entities[e.data];
        const { error } = await auth.supabase.from(def.key).delete().eq("id", id.data);
        if (error) return jsonError(500, error.message);
      ...
      PATCH: async ({ request, params }) => {
        ...
        const { data, error } = await auth.supabase.from(def.key).update(values.data).eq("id", id.data).select().single();
        if (error) return jsonError(500, error.message);
        return jsonResponse(200, { row: data });
      },
    },
