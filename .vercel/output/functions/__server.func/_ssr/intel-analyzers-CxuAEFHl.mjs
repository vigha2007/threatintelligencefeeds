import { e as entry_default } from "../_libs/fuse.js.mjs";
...
function pseudoReports(seed, ceiling) {
  let h = 5381;
  for (let i = 0; i < seed.length; i++) h = (h << 5) + h + seed.charCodeAt(i) >>> 0;
  return ceiling === 0 ? 0 : h % ceiling;
}
function buildPhoneAnalysis(input, api) {
  ...
  let status = trust >= 65 ? "legitimate" : trust >= 40 ? "suspicious" : "scam";
  ...
  const explanation = status === "legitimate" ? `No spam, scam, fraud, robocall, or telemarketing indicators were found. This is a valid ${kind} registered in ${where}. Reputation is unknown.` : status === "suspicious" ? `This number shows risk indicators${reports ? ` and has ${reports} community report${reports === 1 ? "" : "s"}` : ""}. Treat unsolicited contact with caution.` : `Multiple intelligence signals flag this number as high risk. Do not share personal information.`;
  let confidence = 70;
...
function fuse(list) {
  return new entry_default(list.map((v) => ({ v })), {
    keys: ["v"],
    includeScore: true,
    threshold: 0.32,
    ignoreLocation: true,
...
function fuzzyHits(text, terms, f) {
...
function analyzeSms(text) {
  ...
  const phishingSignal = scamHits.length > 0 || phishHits.length > 0 || hasAmount || hasShortener || hasSuspiciousTld;
  if (looksLikeOtp && !phishingSignal) {
    if (hasUrl) {
      return {
        ...
        confidence: 80,
        checkedAt,
        explanation: "This looks like an OTP / verification message, but the inclusion of a URL is unusual for legitimate OTP delivery. Verify the sender before tapping any link.",
        source: "AI Analysis"
      };
    }
    const trust2 = 90;
    return {
      ...
      confidence: 92,
      checkedAt,
      explanation: "This appears to be a legitimate one-time-password / verification message. No scam, phishing, or social-engineering indicators were detected.",
      source: "AI Analysis"
    };
  }
  let risk = 0;
  ...
  const status = trust >= 65 ? "legitimate" : trust >= 40 ? "suspicious" : "scam";
  const threatLevel = risk >= 80 ? "Critical" : risk >= 60 ? "High" : risk >= 35 ? "Medium" : "Low";
  const explanation = category === "Lottery Scam" ? "This message contains lottery / reward / prize language commonly associated with financial fraud." : category === "Banking Fraud" ? "This message impersonates banking or account-management workflows — a pattern used to harvest credentials and one-time passwords." : category === "OTP Scam" ? "This message asks for a verification code alongside phishing indicators. Legitimate institutions will never request your OTP." : category === "Phishing" ? "This message uses click-to-verify lures and suspicious links characteristic of phishing campaigns." : "This message uses pressure tactics, payment requests, or unrealistic offers consistent with social-engineering fraud.";
  return {
...
function statusToSeverity(status, trustScore) {
...
export {
  analyzeSms as a,
  buildPhoneAnalysis as b,
  statusToSeverity as s
};
